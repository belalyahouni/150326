...
if [ "$ROUND" = "unified-latency" ] || [ "$ROUND" = "all" ]; then
  echo "=== 1B Round unified-latency: 1B-unified-from-bad (GPU0, cache=16, no trace) ==="
  PID0=$(start_unified 0 8000 16 test1B_unified_from_bad 0)
  wait_for_server 8000 $PID0 || { shutdown $PID0; exit 1; }
  run_bench_1B 8000 test1B_unified_from_bad_seed${SEED}.json > "$LOGS/bench_test1B_unified_latency_seed${SEED}.log" 2>&1
  B_EX=$?
  echo "[1B-unified-latency] bench exit: $B_EX"
  shutdown $PID0
  echo "=== 1B Round unified-latency done ==="
fi

if [ "$ROUND" = "unified-trace" ] || [ "$ROUND" = "all" ]; then
  echo "=== 1B Round unified-trace: 1B-unified-from-bad (GPU0, cache=16, TRACE=$TRACE_LEVEL) ==="
  PID0=$(start_unified 0 8000 16 test1B_unified_from_bad 1)
  wait_for_server 8000 $PID0 || { shutdown $PID0; exit 1; }
  run_bench_1B 8000 _discard_trace_seed${SEED}.json > "$LOGS/bench_test1B_unified_trace_seed${SEED}.log" 2>&1
  B_EX=$?
  echo "[1B-unified-trace] bench exit: $B_EX"
  shutdown $PID0
  rm -f "$RESULTS/_discard_trace_seed${SEED}.json"
  echo "=== 1B Round unified-trace done ==="
fi
