#!/bin/bash
# Realistic-workload test at M=64. 4 cells (3 static + 1 unified).
set -u
SEED=1
...
PROMPTS=$ROOT/prompts/realistic_prompts.jsonl
...
run_bench() {
  ...
  "$VLLM" bench serve --backend vllm --host 127.0.0.1 --port $port \
    ...
    --custom-output-len 50 --num-prompts 10 \
...
run_cell() {
  ...
  run_bench $port "${cell}_seed${SEED}.json" \
    > "$LOGS/bench_${cell}_seed${SEED}_g${gpu}.log" 2>&1
  echo "[${cell} g${gpu}] bench exit: $?"
  shutdown $PID
}

# All cells at M=64.
echo "=== Realistic workload at M=64 ==="
echo "  Round 1: static C=20 (GPU0) + static C=40 (GPU1) ==="
run_cell realistic_static_C20 0 8000 static 20 44 &
run_cell realistic_static_C40 1 8001 static 40 24 &
wait
echo "  Round 2: static C=60 (GPU0) + unified pool=64 (GPU1) ==="
run_cell realistic_static_C60 0 8000 static 60 4 &
run_cell realistic_unified_init40 1 8001 unified 40 64 &
wait
echo "=== Realistic workload done ==="
